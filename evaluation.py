...
def evaluate_chunk_size(chunk_size, embed):
    ...
    vectordb = Chroma.from_documents(chunks, embed, persist_directory=f"db_{chunk_size}")
    llm = HuggingFacePipeline(
        pipeline=pipeline("text2text-generation", model="google/flan-t5-large", max_new_tokens=200)
    )
    qa = RetrievalQA.from_chain_type(llm=llm, retriever=vectordb.as_retriever(search_kwargs={"k": 5}), return_source_documents=True)
    ...
    for item in dataset:
        ...
        cos = cosine_similarity([embed.embed_query(ans)],[embed.embed_query(gt)])[0][0]
...
def main():
    ...
    print("Evaluation complete → test_results.json generated!")
